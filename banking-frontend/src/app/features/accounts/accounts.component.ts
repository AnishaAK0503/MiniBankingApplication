...
export class AccountsComponent {
  ...
  displayAccountType(value: string): string {
    return value?.toUpperCase() === 'CHECKING' ? 'Current' : value?.toLowerCase() === 'savings' ? 'Savings' : value || '-';
  }
  constructor() {
    afterNextRender(() => {
      this.loadAccounts();
  ...
  private customerId(): number | undefined {
    ...
    return this.customers.find(customer => customer.email.toLowerCase() === user?.email.toLowerCase() || customer.name.toLowerCase() === user?.name.toLowerCase())?.id;
  }

  loadOwnRequests(): void {
    this.api.getAccountRequests(this.customerId()).subscribe({ next: data => { this.requests = data; this.changeDetector.detectChanges(); }, error: err => { this.requestError = this.message(err); this.toast.show(this.requestError, 'error'); this.changeDetector.detectChanges(); } });
  }

  submitAccountRequest(): void {
    const id = this.customerId();
    if (!id) { this.requestError = 'No matching customer profile was found for this login. Ask an Admin to create a Customer record with the same email before requesting an account.'; this.toast.show('Your customer profile could not be found.', 'error'); this.changeDetector.detectChanges(); return; }
    ...
    this.api.createAccountRequest({ customerId: id, ...this.requestForm }).subscribe({
      ...
      error: err => { this.requestSaving = false; this.requestError = this.message(err); this.toast.show(this.requestError, 'error'); this.changeDetector.detectChanges(); }
    });
  }
  ...
  loadAccounts(): void { 
    this.loading = true; 
    this.error = '';
    this.api.getAccounts().pipe(timeout({ each: 5000 }), retry({ count: 4, delay: 1000 })).subscribe({ 
        next: data => { 
  ...
  exportCsv(): void {
    this.csv.download('accounts.csv', ['Account Number', 'Account Holder', 'Type', 'Balance'], this.accounts.map(account => [account.accountNumber, account.customerName || 'Customer', this.displayAccountType(account.accountType), account.balance]));
  }
  ...
  createAccount(): void { 
    ...
    this.error = ''; 
    this.success = ''; 
    this.api.createAccount(this.form).pipe(timeout({ each: 10000 })).subscribe({ 
        next: account => { 
