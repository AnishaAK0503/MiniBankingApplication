...
interface RequestItem { id: number; amount: number; description: string; status: string; createdBy: string; }
...
export class ApprovalsComponent {
  ...
  constructor() { this.load(); this.loadAccountRequests(); }
  ...
  load(): void {
    ...
    this.requests = normalized
      .filter((request: RequestItem) => (request.status ?? '').trim().toUpperCase() === 'PENDING_APPROVAL');
  ...
  loadAccountRequests(): void {
    this.loading = true;
    this.error = '';
    this.api.getPendingAccountRequests().pipe(retry({ count: 2, delay: 500 }), timeout({ each: 5000 })).subscribe({
      next: data => {
        this.accountRequests = data.filter((request: AccountRequest) => (request.status ?? '').trim().toUpperCase() === 'PENDING_APPROVAL');
        this.loading = false;
        this.buildPendingItems();
  ...
  private buildPendingItems(): void {
    const beneficiaryRequests = JSON.parse(localStorage.getItem('mini-banking-beneficiary-requests') ?? '[]')
  ...
  get totalPages(): number { return Math.max(1, Math.ceil(this.pendingItems.length / this.pageSize)); }
  get visiblePendingItems(): PendingApprovalItem[] { const start = (this.currentPage - 1) * this.pageSize; return this.pendingItems.slice(start, start + this.pageSize); }
  get pageStart(): number { return (this.currentPage - 1) * this.pageSize + 1; }
  get pageEnd(): number { return Math.min(this.currentPage * this.pageSize, this.pendingItems.length); }
  goToPage(page: number): void { this.currentPage = Math.min(Math.max(page, 1), this.totalPages); }
  exportCsv(): void { this.csv.download('pending-approvals.csv', ['Type', 'Customer', 'Request', 'Created By', 'Created At', 'Status', 'Remarks'], this.pendingItems.map(item => [item.type, item.customerName, item.displayName, item.createdBy, item.createdAt, item.status, item.remarks || ''])); }

  approveAccount(request: AccountRequest): void {
    this.api.approveAccountRequest(request.id, this.auth.getCurrentUser()?.name ?? 'Checker').subscribe({
      next: () => {
  ...
  submitReject(request: AccountRequest): void {
    ...
    this.api.rejectAccountRequest(request.id, this.auth.getCurrentUser()?.name ?? 'Checker', reason).subscribe({
      next: () => {
        this.accountRequests = this.accountRequests.filter(item => item.id !== request.id);
  ...
  displayType(type: string): string { return type?.toUpperCase() === 'CURRENT' || type?.toUpperCase() === 'CHECKING' ? 'Current Account' : 'Savings Account'; }

  approveItem(item: PendingApprovalItem): void {
    if (item.type === 'account') return this.approveAccount(item.request as AccountRequest);

    const all = JSON.parse(localStorage.getItem(item.type === 'transaction' ? 'mini-banking-transfer-requests' : 'mini-banking-beneficiary-requests') ?? '[]');
    const updated = all.map((request: any) => request.id === item.id ? { ...request, status: 'APPROVED' } : request);
    localStorage.setItem(item.type === 'transaction' ? 'mini-banking-transfer-requests' : 'mini-banking-beneficiary-requests', JSON.stringify(updated));
    this.requests = JSON.parse(localStorage.getItem('mini-banking-transfer-requests') ?? '[]').filter((request: RequestItem) => (request.status ?? '').trim().toUpperCase() === 'PENDING_APPROVAL');
    this.load();
    this.loadAccountRequests();
    this.toast.show(`${item.type === 'transaction' ? 'Transaction' : 'Beneficiary'} approved successfully.`);
  }
  ...
  submitRejectItem(item: PendingApprovalItem): void {
    ...
    const storageKey = item.type === 'transaction' ? 'mini-banking-transfer-requests' : 'mini-banking-beneficiary-requests';
    const all = JSON.parse(localStorage.getItem(storageKey) ?? '[]');
    const updated = all.map((request: any) => request.id === item.id ? { ...request, status: 'REJECTED', rejectionReason: reason } : request);
    localStorage.setItem(storageKey, JSON.stringify(updated));
    this.rejectingRequestId = null;
    this.rejectionReason[item.id] = '';
    this.load();
    this.loadAccountRequests();
    this.toast.show(`${item.type === 'transaction' ? 'Transaction' : 'Beneficiary'} request rejected.`);
  }
  ...
  decide(item: RequestItem, status: 'APPROVED' | 'REJECTED'): void {
    ...
    const updated = all.map((request: RequestItem) => request.id === item.id ? { ...request, status } : request);
    ...
    logs.unshift({ user: 'Current Checker', action: `${status}_REQUEST`, entity: `Transfer request #${item.id}`, time: new Date().toLocaleString() });
    localStorage.setItem('mini-banking-audit-logs', JSON.stringify(logs));
    this.requests = updated.filter((request: RequestItem) => (request.status ?? '').trim().toUpperCase() === 'PENDING_APPROVAL');
    this.buildPendingItems();
