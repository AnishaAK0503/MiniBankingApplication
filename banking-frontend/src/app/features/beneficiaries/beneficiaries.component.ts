...
export class BeneficiariesComponent {
    ...
    afterNextRender(() => {
      ...
      this.api.getCustomers().subscribe({
        next: data => {
          ...
          const customer = data.find(item => item.email.toLowerCase() === user?.email.toLowerCase() || item.name.toLowerCase() === user?.name.toLowerCase());
  ...
  get totalPages(): number { return Math.max(1, Math.ceil(this.beneficiaries.length / this.pageSize)); }
  ...
  get pageEnd(): number { return Math.min(this.currentPage * this.pageSize, this.beneficiaries.length); }

  load(): void {
    this.loading = true;
    this.api.getBeneficiaries().pipe(retry({ count: 4, delay: 1000 }), timeout({ each: 10000 })).subscribe({
      next: data => {
  ...
  exportCsv(): void {
    this.csv.download('beneficiaries.csv', ['Name', 'Account', 'Bank', 'Customer', 'Customer Account'], this.beneficiaries.map(item => [item.name, item.accountNumber, item.bankName, this.customerName(item.customerId), this.customerAccountNumber(item.customerId)]));
  }
  ...
  createBeneficiary(): void {
    ...
    if (this.permissions.isMaker) {
      ...
      const request = {
        id: Date.now(),
        customerId: this.form.customerId || this.customers.find(item => item.email.toLowerCase() === user?.email.toLowerCase() || item.name.toLowerCase() === user?.name.toLowerCase())?.id || 0,
        name: this.form.name,
    ...
    this.error = '';
    this.success = '';
    this.api.createBeneficiary(this.form).pipe(timeout({ each: 10000 })).subscribe({
      next: item => {
  ...
  remove(item: Beneficiary): void {
    ...
    this.api.deleteBeneficiary(item.id).pipe(timeout({ each: 10000 })).subscribe({
      next: () => {
