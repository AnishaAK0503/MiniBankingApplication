...
export class TransferComponent {
  ...
  submit(): void {
    if (!this.form.accountId || !this.form.beneficiaryId || this.form.amount <= 0) {
      this.error = 'Select an account, beneficiary, and valid amount.';
      return;
    }
    ...
    const request = {
      ...
      createdBy: this.auth.getCurrentUser()?.name ?? 'Customer',
    };
  ...
  private writeAudit(action: string, entity: number, user: string): void {
    ...
    logs.unshift({
      user,
      action,
      entity: `Transfer request #${entity}`,
